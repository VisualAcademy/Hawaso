...
{
    /// <summary>
    /// DivisionRepository는 IDivisionRepository와 IDisposable 인터페이스를 구현하는 클래스입니다.
    /// </summary>
    public class DivisionRepository : IDivisionRepository, IDisposable
    {
        private readonly DivisionAppDbContext _context;
        private readonly ILogger _logger;

        /// <summary>
        /// DivisionRepository 생성자는 DB Context와 Logger를 주입받습니다.
        /// </summary>
        public DivisionRepository(DivisionAppDbContext context, ILoggerFactory loggerFactory)
        {
            this._context = context;
            this._logger = loggerFactory.CreateLogger(nameof(DivisionRepository));
        }

        #region [4][1] 입력: AddAsync
        //[4][1] 입력: AddAsync
        /// <summary>
        /// AddAsync 메서드는 신규 DivisionModel 인스턴스를 데이터베이스에 추가합니다.
        /// </summary>
        public async Task<DivisionModel> AddAsync(DivisionModel model)
        {
        ...
        //[4][2] 출력: GetAllAsync
        /// <summary>
        /// GetAllAsync 메서드는 모든 DivisionModel 인스턴스를 데이터베이스에서 가져옵니다.
        /// </summary>
        public async Task<List<DivisionModel>> GetAllAsync()
        {
            return await _context.Divisions.OrderByDescending(m => m.Id).ToListAsync();
        }
        #endregion
        ...
        //[4][3] 상세: GetByIdAsync
        /// <summary>
        /// GetByIdAsync 메서드는 주어진 Id에 해당하는 DivisionModel 인스턴스를 데이터베이스에서 가져옵니다.
        /// </summary>
        public async Task<DivisionModel> GetByIdAsync(long id)
        {
            var model = await _context.Divisions.SingleOrDefaultAsync(m => m.Id == id);
        ...
        //[4][4] 수정: UpdateAsync
        /// <summary>
        /// EditAsync 메서드는 주어진 DivisionModel 인스턴스의 변경사항을 데이터베이스에 반영합니다.
        /// </summary>
        public async Task<bool> EditAsync(DivisionModel model)
        {
            try
            {
                //_context.Divisions.Attach(model);
        ...
        /// <summary>
        /// UpdateAsync 메서드는 주어진 DivisionModel 인스턴스의 변경사항을 데이터베이스에 반영합니다.
        /// </summary>
        public async Task<bool> UpdateAsync(DivisionModel model)
        {
            try
            {
                var m = _context.Divisions.Find(model.Id);
        ...
        //[4][5] 삭제
        /// <summary>
        /// 지정된 ID를 가진 Division를 삭제합니다.
        /// </summary>
        /// <param name="id">삭제할 Division의 ID</param>
        /// <returns>작업 성공 여부</returns>
        public async Task<bool> DeleteAsync(long id)
        {
            try
            {
                var model = await _context.Divisions.FindAsync(id);
        ...
        //[4][6] 페이징: GetAllAsync()
        /// <summary>
        /// 모든 Division를 페이지 단위로 반환합니다.
        /// </summary>
        /// <param name="pageIndex">반환할 페이지 인덱스</param>
        /// <param name="pageSize">페이지 당 반환할 개수</param>
        /// <returns>페이지 결과</returns>
        public async Task<PagingResult<DivisionModel>> GetAllAsync(int pageIndex, int pageSize)
        {
            var totalRecords = await _context.Divisions.CountAsync();
            var models = await _context.Divisions
                .OrderByDescending(m => m.Id)
                .Skip(pageIndex * pageSize)
                .Take(pageSize)
                .ToListAsync();

            return new PagingResult<DivisionModel>(models, totalRecords);
        }
        #endregion
        ...
        //[4][7] 부모
        /// <summary>
        /// 지정된 부모 ID를 가진 Division를 페이지 단위로 반환합니다.
        /// </summary>
        /// <param name="pageIndex">반환할 페이지 인덱스</param>
        /// <param name="pageSize">페이지 당 반환할 개수</param>
        /// <param name="parentId">부모 ID</param>
        /// <returns>페이지 결과</returns>
        public async Task<PagingResult<DivisionModel>> GetAllByParentIdAsync(
            int pageIndex,
            int pageSize,
            int parentId)
        {
            var totalRecords = await _context.Divisions
        ...
        //[4][8] 상태
        /// <summary>
        /// 지정된 부모 ID를 가진 Division의 상태를 반환합니다.
        /// </summary>
        /// <param name="parentId">부모 ID</param>
        ...
        public async Task<Tuple<int, int>> GetStatus(int parentId)
        {
            var totalRecords = await _context.Divisions
                //.Where(m => m.ParentId == parentId)
                .CountAsync();
            var pinnedRecords = await _context.Divisions
        ...
        //[4][9] 부모 삭제
        /// <summary>
        /// 지정된 부모 ID를 가진 모든 Division를 삭제합니다.
        /// </summary>
        /// <param name="parentId">부모 ID</param>
        /// <returns>작업 성공 여부</returns>
        ...
        //[4][10] 검색
        /// <summary>
        /// 주어진 쿼리로 Division를 검색하고, 그 결과를 페이지 단위로 반환합니다.
        /// </summary>
        /// <param name="pageIndex">반환할 페이지 인덱스</param>
        /// <param name="pageSize">페이지 당 반환할 개수</param>
        /// <param name="searchQuery">검색 쿼리</param>
        /// <returns>페이지 결과</returns>
        public async Task<PagingResult<DivisionModel>> SearchAllAsync(
            int pageIndex,
            int pageSize,
            string searchQuery)
        {
        ...
        //[4][11] 부모 검색
        /// <summary>
        /// 주어진 쿼리로 지정된 부모 ID를 가진 Division를 검색하고, 그 결과를 페이지 단위로 반환합니다.
        /// </summary>
        /// <param name="pageIndex">반환할 페이지 인덱스</param>
        /// <param name="pageSize">페이지 당 반환할 개수</param>
        /// <param name="searchQuery">검색 쿼리</param>
        /// <param name="parentId">부모 ID</param>
        /// <returns>페이지 결과</returns>
        public async Task<PagingResult<DivisionModel>> SearchAllByParentIdAsync(
            int pageIndex,
            int pageSize,
            string searchQuery,
            int parentId)
        ...
        public async Task<SortedList<int, double>> GetMonthlyCreateCountAsync()
        {
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                var cnt = _context.Divisions.AsEnumerable()
                    .Where(
                        m => m?.CreatedAt != null
                        //&&
        ...
        /// </summary>
        public async Task<PagingResult<DivisionModel>> GetAllByParentKeyAsync(
            int pageIndex,
            int pageSize,
            string parentKey)
        {
        ...
        /// </summary>
        public async Task<PagingResult<DivisionModel>> SearchAllByParentKeyAsync(
            int pageIndex,
            int pageSize,
            string searchQuery,
            string parentKey)
        ...
        /// </summary>
        public async Task<ArticleSet<DivisionModel, int>> GetAllAsync<TParentIdentifier>(int pageIndex, int pageSize, string searchField, string searchQuery, string sortOrder, TParentIdentifier parentIdentifier)
        {
            var items = _context.Divisions.AsQueryable();
            ...
            return new ArticleSet<DivisionModel, int>(await items.AsNoTracking().ToListAsync(), totalCount);
        }
        #endregion

        #region  [4][15][2] 리스트(페이징, 검색, 정렬): GetArticlesAsync<TParentIdentifier>()
        public async Task<ArticleSet<DivisionModel, int>> GetArticlesAsync<TParentIdentifier>(
            int pageIndex,
            int pageSize,
            string searchField,
            string searchQuery,
        ...
        /// </summary>
        public async Task<DivisionModel> AddAsync(DivisionModel model, int parentRef, int parentStep, int parentRefOrder)
        {
            model.CreatedAt = DateTime.UtcNow;
        ...
        /// </summary>
        public async Task<DivisionModel> AddAsync(DivisionModel model, int parentId)
        {
            model.CreatedAt = DateTime.UtcNow;
        ...
        /// </summary>
        public async Task<ArticleSet<DivisionModel, long>> GetByAsync<TParentIdentifier>(
            FilterOptions<TParentIdentifier> options)
        {
            ...
            return new ArticleSet<DivisionModel, long>(await items.AsNoTracking().ToListAsync(), totalCount);
        }
        #endregion
        ...
        public Task<ArticleSet<DivisionModel, int>> GetAllAsync<TParentIdentifier>(int pageIndex, int pageSize, string searchField, string searchQuery, string sortOrder, TParentIdentifier parentIdentifier, DateTime from, DateTime to)
        {
