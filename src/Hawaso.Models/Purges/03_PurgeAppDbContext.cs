...
namespace Hawaso.Models
{
    ...
    /// [2] DbContext Class
    /// </summary>
    public class PurgeAppDbContext : DbContext
    {
        ...
        public PurgeAppDbContext() : base() 
        {
            // Empty
            // ChangeTracker.QueryTrackingBehavior = QueryTrackingBehavior.NoTracking;
        }

        public PurgeAppDbContext(DbContextOptions<PurgeAppDbContext> options)
            : base(options)
        {
            // 공식과 같은 코드, 교과서다운 코드
            // ChangeTracker.QueryTrackingBehavior = QueryTrackingBehavior.NoTracking;
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Purge>().Property(m => m.Created).HasDefaultValueSql("GetDate()");
            modelBuilder.Entity<Purge>().Property(m => m.PostDate).HasDefaultValueSql("GetDate()");

            modelBuilder.Entity<PurgeArchive>().Property(m => m.PurgeDate).HasDefaultValueSql("GetDate()");
        }

        //[!] MemoApp 솔루션 관련 모든 테이블에 대한 참조 
        public DbSet<Purge> Purges { get; set; } // = null!;

        public DbSet<PurgeArchive> PurgesArchives { get; set; }
    }
}
