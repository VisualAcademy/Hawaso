...
namespace Hawaso.Models
{
    ...
    /// </summary>
    public class PurgeRepository : IPurgeRepository, IDisposable
    {
        private readonly PurgeAppDbContext _context;
        private readonly ILogger _logger;

        public PurgeRepository(PurgeAppDbContext context, ILoggerFactory loggerFactory)
        {
            this._context = context;
            this._logger = loggerFactory.CreateLogger(nameof(PurgeRepository));
        }

        #region [4][1] 입력: AddAsync
        //[4][1] 입력: AddAsync
        public async Task<Purge> AddAsync(Purge model)
        {
            #region 답변 기능 추가
            // 현재테이블의 Ref의 Max값 가져오기
            int maxRef = 1;
            int? max = await _context.Purges.DefaultIfEmpty().MaxAsync(m => m == null ? 0 : m.Ref);
            if (max.HasValue)
            {
                maxRef = (int)max + 1;
            }
        ...
        //[4][2] 출력: GetAllAsync
        public async Task<List<Purge>> GetAllAsync()
        {
            // 학습 목적으로... InMemory 데이터베이스에선 사용 금지 
            //return await _context.Memos.FromSqlRaw<Purge>("Select * From dbo.Memos Order By Id Desc") 
            return await _context.Purges.OrderByDescending(m => m.Id)
        ...
        //[4][3] 상세: GetByIdAsync
        public async Task<Purge> GetByIdAsync(long id)
        {
            var model = await _context.Purges
                //.Include(m => m.MemosComments)
        ...
        //[4][4] 수정: UpdateAsync
        public async Task<bool> EditAsync(Purge model)
        {
            try
            {
        ...
        public async Task<bool> UpdateAsync(Purge model)
        {
            try
            {
                // 강의에서는 AutoMapper NuGet 사용 X

                var old = _context.Purges.Find(model.Id);
        ...
        public async Task<bool> DeleteAsync(long id)
        {
            ...
            try
            {
                var model = await _context.Purges.FindAsync(id);
        ...
        //[4][6] 페이징: GetAllAsync()
        public async Task<PagingResult<Purge>> GetAllAsync(int pageIndex, int pageSize)
        {
            var totalRecords = await _context.Purges.CountAsync();
            var models = await _context.Purges
            ...
            return new PagingResult<Purge>(models, totalRecords);
        }
        #endregion

        #region [4][7] 부모: GetAllByParentIdAsync() 
        //[4][7] 부모
        public async Task<PagingResult<Purge>> GetAllByParentIdAsync(
            int pageIndex,
            int pageSize,
            int parentId)
        {
            var totalRecords = await _context.Purges
        ...
        public async Task<Tuple<int, int>> GetStatus(int parentId)
        {
            var totalRecords = await _context.Purges
                .Where(m => m.ParentId == parentId)
                .CountAsync();
            var pinnedRecords = await _context.Purges
        ...
        //[4][10] 검색
        public async Task<PagingResult<Purge>> SearchAllAsync(
            int pageIndex,
            int pageSize,
            string searchQuery)
        {
        ...
        //[4][11] 부모 검색
        public async Task<PagingResult<Purge>> SearchAllByParentIdAsync(
            int pageIndex,
            int pageSize,
            string searchQuery,
            int parentId)
        ...
        public async Task<SortedList<int, double>> GetMonthlyCreateCountAsync()
        {
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                var cnt = _context.Purges.AsEnumerable()
                    .Where(
                        m => m.Created != null
                        &&
        ...
        //[4][13] 부모 페이징
        public async Task<PagingResult<Purge>> GetAllByParentKeyAsync(
            int pageIndex,
            int pageSize,
            string parentKey)
        {
        ...
        //[4][14] 부모 검색
        public async Task<PagingResult<Purge>> SearchAllByParentKeyAsync(
            int pageIndex,
            int pageSize,
            string searchQuery,
            string parentKey)
        ...
        //[4][15] 리스트(페이징, 검색, 정렬)
        public async Task<ArticleSet<Purge, int>> GetArticlesAsync<TParentIdentifier>(
            int pageIndex,
            int pageSize,
            string searchField,
            string searchQuery,
            ...
            TParentIdentifier parentIdentifier)
        {
            ...
            return new ArticleSet<Purge, int>(await items.AsNoTracking().ToListAsync(), totalCount);
        }
        #endregion

        //[4][16] 답변: ReplyApp, AddAsync()
        public async Task<Purge> AddAsync(Purge model, int parentRef, int parentStep, int parentRefOrder)
        {
            #region 답변 관련 기능 추가된 영역
            // 비집고 들어갈 자리: 부모글 순서보다 큰 글이 있다면(기존 답변 글이 있다면) 해당 글의 순서를 모두 1씩 증가 
            var replys = await _context.Purges
        ...
        // TODO: 답변 로직 수정할 것...
        public async Task<Purge> AddAsync(Purge model, int parentId)
        {
            #region 답변 관련 기능 추가된 영역
            //[0] 변수 선언
            var maxRefOrder = 0;
            ...
            var tempMaxRefOrder = await _context.Purges.Where(m => m.ParentNum == parentId).DefaultIfEmpty().MaxAsync(m => m == null ? 0 : m.RefOrder);
            var sameGroup = await _context.Purges.Where(m => m.ParentNum == parentId && m.RefOrder == tempMaxRefOrder).FirstOrDefaultAsync();
            if (sameGroup != null)
            {
            ...
            {
                var tmpParent = await _context.Purges.Where(m => m.Id == parentId).SingleOrDefaultAsync();
                if (tmpParent != null)
                {
            ...
            var replys = await _context.Purges.Where(m => m.Ref == parentRef && m.RefOrder > (maxRefOrder + maxAnswerNum)).ToListAsync();
            foreach (var item in replys)
            {
        ...
        //[4][6] 검색: GetByAsync()
        public async Task<ArticleSet<Purge, long>> GetByAsync<TParentIdentifier>(
            FilterOptions<TParentIdentifier> options)
        {
            //var items = from m in _context.Memos select m; // 쿼리 구문(Query Syntax)
            //var items = _context.Memos.Select(m => m); // 메서드 구문(Method Syntax)
            var items = _context.Purges.AsQueryable(); // IQueryable<T>: Expressoin Tree 생성(Deferred Execution)
            ...
            // AsNoTracking() 사용으로 성능 향상: https://docs.microsoft.com/ko-kr/ef/core/querying/tracking#no-tracking-queries
            return new ArticleSet<Purge, long>(await items.AsNoTracking().ToListAsync(), totalCount);
        }
        #endregion
